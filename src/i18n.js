...
export const copy = {
  pt: {
    ...
    projects:[
      {type:"Auditoria com IA", title:"Auditoria automática de balancetes", metric:"294 testes automatizados", detail:"com as duas integrações validadas ponta a ponta contra dados reais", text:"Monitora o encerramento do fechamento, extrai as peças contábeis por automação de interface e gera um relatório técnico com IA, sempre com revisão humana obrigatória antes de sair. Roda em lote a 47 segundos por empresa, e o motor foi escrito genérico para ser reaproveitado em outras auditorias.", stack:["Python","pywinauto","PostgreSQL","IA"]},
      {type:"Conferência fiscal", title:"Conferência de INSS judicial", metric:"0 divergência em 39.634 valores", detail:"132 competências reconferidas, com o depósito batendo exato contra a fonte", text:"Reconstrói onze anos de competências a partir das planilhas originais de 15 estabelecimentos e marca cada valor que divergia do levantamento anterior. O achado que justificou o projeto: em 17 combinações de aba e ano o total fechava, mas os valores estavam trocados entre eventos, então nenhuma conferência por total pegaria o erro.", stack:["Python","openpyxl","Excel","Testes automatizados"]},
      {type:"Análise documental em escala", title:"Triagem de retenções em notas", metric:"2.191 notas reclassificadas", detail:"ao trocar preenchimento por acerto como critério, numa base de 9.571 documentos", text:"Lê o PDF por geometria, e não como texto plano, e classifica cada nota por semáforo. O ganho veio de medir acerto em vez de só preenchimento: o índice vermelho subiu de propósito quando o sistema parou de atribuir ao prestador o único município impresso na nota. Um de-para de CNAE levou o acerto do código de 75,7% para 79,9%.", stack:["Python","PyMuPDF","PySide6"]},
  ...
  en: {
    ...
    projects:[
      {type:"AI-assisted audit", title:"Automated trial balance audit", metric:"294 automated tests", detail:"with both integrations validated end to end against real data", text:"Watches for the monthly close to be signed off, pulls the accounting reports through UI automation and produces a technical report with AI, always with mandatory human review before it goes out. It runs in batch at 47 seconds per company, and the engine was written to be generic enough for other audits.", stack:["Python","pywinauto","PostgreSQL","AI"]},
      {type:"Tax reconciliation", title:"Court-ordered social security review", metric:"0 variances across 39,634 values", detail:"132 monthly periods rechecked, with the deposit matching the source exactly", text:"Rebuilds eleven years of periods from the original spreadsheets of 15 establishments and flags every value that diverged from the previous survey. The finding that justified the project: in 17 combinations of sheet and year the total matched while the values were swapped between line items, so no total-level check would ever have caught it.", stack:["Python","openpyxl","Excel","Automated tests"]},
      {type:"Document analysis at scale", title:"Withholding triage on invoices", metric:"2,191 invoices reclassified", detail:"after switching the criterion from fill rate to accuracy, across 9,571 documents", text:"Reads the PDF by geometry rather than as flat text and sorts each invoice into a traffic light. The gain came from measuring accuracy instead of fill rate alone: the red count went up on purpose once the system stopped assigning the supplier the only city printed on the invoice. A CNAE mapping took code accuracy from 75.7% to 79.9%.", stack:["Python","PyMuPDF","PySide6"]},
